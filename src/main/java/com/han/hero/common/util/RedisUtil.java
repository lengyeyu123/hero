...
public class RedisUtil {
    ...
    private static final String err = "RedisUtil 工具类发生异常";
    ...
    public void del(String... keys) {
        ...
        if (keys != null && keys.length > 0) {
            for (String key : keys) {
                ...
                if (set != null && !key.isEmpty()) {
                    keySet.addAll(set);
                }
            }
            delCount = redisTemplate.delete(keySet);
        }
        log.debug("--------------------------------------------");
        log.debug("成功删除缓存：" + keySet);
        log.debug("缓存删除数量：" + delCount + "个");
        log.debug("--------------------------------------------");
    }
